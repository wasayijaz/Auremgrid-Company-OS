...
safety/proactive lifecycle/durable orchestration/action descriptors, and injected read-only Stripe Billing/accounting
...
- `python -m unittest tests.test_agent_level_routing tests.test_private_single_host_deploy`
...
verification must report schema 47 from `/health` or `/health/detailed`.
