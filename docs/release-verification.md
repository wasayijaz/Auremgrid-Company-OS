...
| 20. Tests prove isolation, persistence, permissions, and workflows | 426 offline behavior tests |
...
| 29. Enabled live read synchronization is restart-safe and honest | Slack/ClickUp/Google Drive/Gmail/Figma account and permission verification, explicit workspace mappings, durable provider cursors/inbox/dedupe, baseline-first backfill, Drive reconciliation/descendant tasks, Gmail label lifecycle, version-fenced exact-file Figma polling/tombstones and bounded frame/section evidence with the parent file as the only lifecycle/object-count record, no Figma comments/version history/review or approval workflows/auto-created deliverables, reviews, or tasks, Fireflies disabled, opaque cross-workspace quarantine, canonical ingestion, provider-aware retry timing, raw-DB secret scans, and no caller-controlled connected state |
