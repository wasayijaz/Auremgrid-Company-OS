...
| 29. Enabled live read synchronization is restart-safe and honest | Slack/ClickUp/Google Drive/Gmail/Figma account and permission verification, explicit workspace mappings, durable provider cursors/inbox/dedupe, baseline-first backfill, Drive reconciliation/descendant tasks, Gmail label lifecycle, version-fenced exact-file Figma polling/tombstones, bounded frame/section evidence, and optional bounded named-version evidence with the parent file as the only lifecycle/object-count record; no Figma comments/review or approval workflows/auto-created deliverables, reviews, or tasks; Fireflies disabled; opaque cross-workspace quarantine, canonical ingestion, provider-aware retry timing, raw-DB secret scans, and no caller-controlled connected state |
