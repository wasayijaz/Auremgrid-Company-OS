...
| 34. Private single-host packaging stays bounded | Dockerfile and Compose static tests prove Python 3.12 packaging, non-root runtime, private loopback proxy defaults, Caddy upstream alignment, no-secret env example, and secret/test exclusions; Python smoke rehearsal proves health, worker, backup/verify, and restore recovery/outbound-off without Docker; these checks do not claim managed hosting, browser runtime automation, or provider connectivity |
...
verification must report schema 56 from `/health` or `/health/detailed`; if the
