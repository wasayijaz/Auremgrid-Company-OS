...
Read-only Stripe Billing/accounting, Meta Ads, Google Ads, and CRM adapters
normalize injected provider pages into immutable provider import records with
...
ad groups and ads remain provider-ledger records only. CRM sync supports only
`contacts` and `opportunities`, mapping valid records into the existing local
CRM/sales tables. They are not live registered connector entries, do not appear
as `live_enabled` catalog connectors, and never send or mutate provider data.
