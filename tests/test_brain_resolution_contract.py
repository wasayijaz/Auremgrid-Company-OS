...
class BrainResolutionContractTests(unittest.TestCase):
    ...
    def test_schema18_replay_preserves_entity_and_alias_rows(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(second.store.schema_version,21); self.assertIsNotNone(second.store.conn.execute("SELECT 1 FROM entities WHERE id=?",(entity["id"],)).fetchone()); self.assertIsNotNone(second.store.conn.execute("SELECT 1 FROM entity_resolution_decisions LIMIT 1").fetchone()) if False else None; second.close()
