...
Schema 46 persists append-only Intelligence orchestration traces for durable specialist review results. Schema 47 stores supervised reversible action descriptors and their originating orchestration trace on agent tasks. It never runs unapproved, one-way, external-send, connector-write, or arbitrary descriptors; the supported execution path is limited to approved same-scope `generate_report` descriptors through the normal report route.
...
fallback, and the schema-42-to-47 Intelligence completion surfaces. See
...
- Docker and Compose files are private single-host packaging templates. They verify build/static boundaries and loopback proxy defaults, not managed hosting, live browser runtime behavior, or provider connectivity.
