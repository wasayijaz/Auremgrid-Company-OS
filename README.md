...
Schema 46 persists append-only Intelligence orchestration traces for durable specialist review results. Schema 47 stores supervised reversible action descriptors and their originating orchestration trace on agent tasks. Schema 48 adds the append-only execution ledger for approved reversible agent actions. The exact supervised catalog is `generate_report` (`report.generate`), `create_notification` (`notification.create`), and `acknowledge_attention` (`proactive_attention.acknowledge`). Execution requires a safe, non-one-way descriptor from that catalog, an approved same-organization/workspace approval request with a matching action kind and canonical payload, and a scoped idempotency key. Rejected or otherwise non-approved approval records block new execution; completed execution rows remain ledgered and are not silently erased. Providers cannot execute writes directly, and the local execution bridge never runs unapproved, one-way, external-send, connector-write, or arbitrary descriptors.
