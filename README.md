...
4. **Minutes 15–20 — Inspect a workflow.** Use the workflow REST/MCP endpoints to list the neutral templates, create a `landing_page` or `campaign_launch` run, start a ready stage, attach evidence, and then return to the dashboard to see the canonical read-only stage board. Brain shows pending proposals, unresolved conflicts, current truths, and semantic/graph health; workflow mutation remains capability-gated through API/MCP.
