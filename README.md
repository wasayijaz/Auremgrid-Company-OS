...
Schema 46 persists append-only Intelligence orchestration traces for durable specialist review results. Schema 47 stores supervised reversible action descriptors and their originating orchestration trace on agent tasks. Schema 48 adds the append-only execution ledger for approved reversible agent actions. The exact supervised catalog is `generate_report` (`report.generate`), `create_notification` (`notification.create`), `acknowledge_attention` (`proactive_attention.acknowledge`), `create_risk` (`risk.create`), `add_work_comment` (`work.comment.create`), and `create_proposal` (`brain.proposal.create`). Execution requires a safe, non-one-way descriptor from that catalog, an approved same-organization/workspace approval request with a matching action kind and canonical payload, and a scoped idempotency key. Rejected or otherwise non-approved approval records block new execution; completed execution rows remain ledgered and are not silently erased. Failed execution rows fence same-key replay until an operator creates a new approved task or idempotency key, and agent run detail exposes the recorded result/error plus replay boundary. Providers cannot execute writes directly, and the local execution bridge never runs unapproved, one-way, external-send, connector-write, or arbitrary descriptors.
...
The default provider is a deterministic, offline lexical fallback so a new installation works without model files or network access. Its health explicitly reports `fallback_used=true`. Schema 16 stores rebuildable float32 vectors in `document_embedding_projection`, scoped by workspace, document, provider, model, version, and dimensions. Retrieval rejects empty or oversized queries, caps effective search limits at 64 results, bounds query text to 2,000 characters, authorizes active source/document IDs before either channel runs, and reports authorized source/document counts plus channel health in metadata. Semantic candidates are never gated by an FTS hit and are rehydrated from canonical SQLite rows before ranking; each result exposes a stable `citation_ref` such as `document:<id>` or `fact:<id>`. An opted-in provider outage is reported as `semantic=degraded` with `fallback_used=false` while cited FTS evidence and startup remain available.
...
For private-host packaging, copy `.env.example` to `.env`, edit the organization id and paths, and keep provider secrets in environment variables or a vault. Run `python scripts/private_host_smoke.py` to rehearse health, one worker job, backup/verify, and restore recovery mode with outbound dispatch disabled; the rehearsal is Python-only and does not require Docker.
...
fallback, and the schema-42-to-48 Intelligence completion surfaces. See
...
- Expert orchestration, recommendation learning, shadow evaluation safety, proactive attention lifecycle, and approved reversible action execution are local service/API/MCP surfaces. They do not provide a hosted model marketplace, autonomous specialist execution, production workflow routing changes, or external action execution. Safe action descriptors are limited to `generate_report`, `create_notification`, `acknowledge_attention`, `create_risk`, `add_work_comment`, and `create_proposal`; they bridge to existing canonical routes only after matching approval, scope, payload, idempotency, and execution-ledger checks.
